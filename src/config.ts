...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    ...
    loginRequestTimeout: Schema.number().min(30).max(5 * 60).step(1).default(60).description('登录请求超时判断边界 (单位：秒)'),
    timeout: Schema.number().min(1 * 1000).max(30 * 1000).step(500).default(15 * 1000).description('WebSocket 握手超时时间（单位：毫秒）'),
